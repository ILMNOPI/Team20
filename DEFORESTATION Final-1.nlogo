...
patches-own
  [forestation
  is-there-forest?]
globals [new_person count_up]

to setup
  clear-all
  ask patches[set pcolor white]
  set-default-shape people "person"
  set-default-shape buildings "building"
   set new_person 0
  ask patches [
    set forestation 0
    set is-there-forest? false]
  construct
  ask patches [forestify]
  create-people initial-population [
    setxy random-pxcor random-pycor
    set age 4.55
    set color orange]

  create-buildings initial-buildings [
    setxy random-pxcor random-pycor
    set size 2.5
    set age 5]
  reset-ticks
...
to go
  ...
  ask people [
    ...
    build
    death]
diffuse forestation 0.8
  ask patches [ forestify ]
  ask buildings [
    clean
    death]
 tick
 set count_up (count_up + 1)
 If count_up = Stop-at [ stop ]
end
...
to forestify
  if is-there-forest? [
    set pcolor green
    set forestation forestation-rate]
  set pcolor scale-color green (forestation - .1) 5 0
end
to construct
  ask n-of forest patches [
    set is-there-forest? true]
end
to clean
  set pcolor yellow + 3
  set forestation max (list 0 (forestation - 1))
  ask neighbors [
   set forestation max (list 0 (forestation - .5))
  ]
  set age age - 0.1
end
to wander-around
...
to build
  if random-float 1 < building-rate [
    hatch-buildings 1 [
      set age 5
      set size 2.5]]
end
...
to reproduce
  if age > 4.36 and random-float 1 < birth-rate [
    hatch-people 1 [set age 5.2
      set new_person (new_person + 1)
      set color orange]
  ]
end
...
to dwell
  if new_person = planting-trees-per-people [
    plant
    set new_person 0]
end
...
to no-oxygen
  if (sum [forestation] of patches * 500 / count people) < 800 [set birth-rate birth-rate - 0.0000001]
  if (sum [forestation] of patches * 500 / count people) < 600 [set birth-rate birth-rate - 0.0000002]
  if (sum [forestation] of patches * 500 / count people) < 200 [set birth-rate birth-rate - 0.0000003]
  if (sum [forestation] of patches * 500 / count people) < 200 [set birth-rate birth-rate - 0.0000005]
  if (sum [forestation] of patches * 500 / count people) > 800 [set birth-rate birth-rate + 0.0000001]
  if (sum [forestation] of patches * 500 / count people) > 900 [set birth-rate birth-rate + 0.0000001]
  if (sum [forestation] of patches * 500 / count people) > 1000 [set birth-rate birth-rate + 0.0000001]
  if (sum [forestation] of patches * 500 / count people) > 1500 [set birth-rate birth-rate + 0.0000005]
  if (sum [forestation] of patches * 500 / count people) > 2000 [set birth-rate birth-rate + 0.0000005]
end
;gender
;affect of trees on population
@#$#@#$#@
GRAPHICS-WINDOW
295
14
...
"" ""
PENS
"People" 1.0 0 -817084 true "" "plot (count people) * 7400"
"Buildings" 1.0 0 -7500403 true "" "plot (count buildings) * 7400"
...
65
count trees
(sum [forestation] of patches)* 3200000
1
1
11
...
135
human to trees
sum [forestation] of patches * 500 / count people
17
1
11
...
"" ""
PENS
"Trees" 1.0 0 -13840069 true "plot (sum [forestation] of patches)* 3200000" "plot (sum [forestation] of patches)* 3200000"
...
false
1
Rectangle -7500403 true false 45 15 255 285
Rectangle -16777216 true false 120 210 180 285
Rectangle -11221820 true false 75 45 135 90
Rectangle -11221820 true false 165 120 225 165
Rectangle -11221820 true false 75 120 135 165
Rectangle -11221820 true false 165 45 225 90
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="true">
    ...
    <go>go</go>
    <timeLimit steps="554"/>
    <metric>count buildings * 7400</metric>
    <metric>count people * 7400</metric>
    <metric>(sum [forestation] of patches)* 3200000</metric>
    <enumeratedValueSet variable="planting-trees-per-people">
      <value value="1000"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="building-rate">
      <value value="0.007"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="forest">
      <value value="833"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="forestation-rate">
      <value value="6"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-buildings">
      <value value="50"/>
    </enumeratedValueSet>
